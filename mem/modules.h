    ...
    public:
        mem::addr_t find_pattern(std::string_view pattern) {
            ...
            for (unsigned long i = 0ul; i < image_size - pattern_size; i++) {
                ...
                std::printf("Found '%s' sig from %s at 0x%x [rva: 0x%x]", pattern.data(), m_name, static_cast<std::uintptr_t>(ret), i);
                return ret;
            }

            std::printf("Signature from %s '%s' not found", m_name, pattern.data());
            return mem::addr_t();
        }
    protected:
